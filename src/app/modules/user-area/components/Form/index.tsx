...
type UserAreaFormFiles = {
    //resolutionFile?: CloudFile,
    //programFile?: CloudFile
}

type Props = {
    submitAction: (data: UserAreaFormField) => unknown
    formId: string
    initialData?: UserAreaFormField
    initialFiles?: UserAreaFormFiles,
    upLoading?: () => void,
}
const UserAreaForm: React.FC<Props> = ({submitAction, initialFiles, upLoading, formId, initialData}: Props) => {
    ...
    const defaultValues: UserAreaFormField | {} = initialData ? {
        ...initialData
    } : {isActive: true};

    const {handleSubmit, control, formState: {errors, dirtyFields, isValid}} = useForm<UserAreaFormField>({
        mode: 'onChange',
        // @ts-ignore
    ...
    //const [files, setFiles] = React.useState<{ [K in keyof UserAreaFormFiles]?: File }>({});
    ...
    const onSubmit = (data: UserAreaFormField) => {
