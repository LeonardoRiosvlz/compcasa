...
import __name__Form, {__name__FormField} from '../Form';
import __name__Details from '../Details';
...
import {__name__Repository} from "api/repositories/__name__(kebabCase)/__name__(kebabCase).repository";
...
type Props = {}
const __name__Table: React.FC<Props> = (props: Props) => {
    ...
    const __name__CamelCase__Repo = useRepository(__name__Repository)
    ...
    const selectedItem = _.first(selected) as __name__;
    ...
    React.useEffect(() => {
        // @ts-ignore
        dispatch(setContext(App_Modules.__name__));
    }, []);
    ...
    const fetch = () => {
        ...
        __name__CamelCase__Repo.findPaginated(
            {
                input: {
            ...
            }).then((res: IPaginatedEntity<__name__>) => {
            // @ts-ignore
    ...
    const addAction = (formData: __name__FormField) => {
        setCrudLoading({...crudLoading, add: true});
        __name__CamelCase__Repo.create({input: {...formData}}).then(() => {
    ...
    const editAction = (formData: __name__FormField) => {
        setCrudLoading({...crudLoading, edit: true});

        const toUpdateData = FormUtils.toUpdateValue<__name__FormField, __name__>(formData, selectedItem);

        __name__CamelCase__Repo.update({
            input: {
    ...
    const removeAction = () => {
        ...
        __name__CamelCase__Repo.delete({input: {entityId: selectedItem?.id}}).then(() => {
    ...
    const removeMultipleAction = () => {
        ...
        __name__CamelCase__Repo.deleteMany({input: {where: {id: {in: ids}}}}).then(() => {
            setCrudLoading({...crudLoading, delete: false});
    ...
    const rows: Array<RowsDef<__name__>> = [
        {
            id: 'name',
            align: 'left',
            sort: true,
    ...
    const formId = {
        edit: `edit-__name__(kebabCase)`,
        add: `add-__name__(kebabCase)`
    };
    ...
    return (<>
            <div className='w-full flex flex-col'>
                <GenericTable rows={rows}
                              ...
                              title={t('common:__name__ConstantCase__')}
            ...
            <GenericModal onClose={() => handleCloseDialog('view')}
                ...
                <div className='w-full p-4'>
                    <__name__Details row={selectedItem}/>
                </div>
            </GenericModal>
            ...
            <GenericModal maxWidth={'sm'} minHeight={200}
                ...
                <div className='w-full p-4 mt-16'>
                    <__name__Form
                        ...
                        initialData={{
                        ...FormUtils.setInitialData<__name__, __name__FormField>(selectedItem)
                    }}
