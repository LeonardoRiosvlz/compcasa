...
type __name__FormFiles = {
    //resolutionFile?: CloudFile,
    //programFile?: CloudFile
}

type Props = {
    submitAction: (data: __name__FormField) => unknown
    formId: string
    initialData?: __name__FormField
    initialFiles?: __name__FormFiles,
    upLoading?: () => void,
}
const __name__Form: React.FC<Props> = ({submitAction, initialFiles, upLoading, formId, initialData}: Props) => {
    ...
    const defaultValues: __name__FormField | {} = initialData ? {
        ...initialData
    } : {isActive: true};

    const {handleSubmit, control, formState: {errors, dirtyFields, isValid}} = useForm<__name__FormField>({
        mode: 'onChange',
        // @ts-ignore
    ...
    //const [files, setFiles] = React.useState<{ [K in keyof __name__FormFiles]?: File }>({});
    ...
    const onSubmit = (data: __name__FormField) => {
